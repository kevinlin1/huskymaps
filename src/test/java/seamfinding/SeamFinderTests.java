...
public abstract class SeamFinderTests {
     ...
     */
    private SeamFinder seamFinder = createSeamFinder();
    ...
    void precomputedImages(String basename) throws IOException {
        ...
        assertValidMinimumEnergySeam(picture, basename, "horizontal");
        assertValidMinimumEnergySeam(picture, basename, "vertical");
    }

    private void assertValidMinimumEnergySeam(Picture picture, String basename, String orientation) throws FileNotFoundException {
        double referenceEnergy = precomputedEnergy(basename, orientation);
        List<Integer> testingSeam;
        double testingEnergy;
        if (orientation.equals("horizontal")) {
            testingSeam = seamFinder.findHorizontal(picture, f);
            checkHorizontal(picture, testingSeam);
            testingEnergy = energyHorizontal(picture, testingSeam);
        } else {
            testingSeam = seamFinder.findVertical(picture, f);
            checkHorizontal(picture.transposed(), testingSeam);
            testingEnergy = energyHorizontal(picture.transposed(), testingSeam);
        }
        assertEquals(referenceEnergy, testingEnergy, EPSILON, () -> String.format(
                "\n" +
                "%s seam\n" +
                "  expected energy: %s\n" +
                "  actual energy:   %s\n" +
                "  actual seam:     %s",
                orientation, referenceEnergy, testingEnergy, testingSeam));
    }
    ...
    static void runtimeExperiments(Supplier<SeamFinder> constructor) {
        SeamFinder seamFinder = constructor.get();
     ...
     */
    private static Picture randomPicture(int width, int height, SplittableRandom spRandom) {
